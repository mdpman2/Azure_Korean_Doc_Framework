...
# tests/ 하위에서 실행되므로 상위(프로젝트 루트) 디렉토리를 import 경로에 추가
...
def test_llm_cache():
    ...
    try:
        ...
        # 2-6. invalidate (비동기 디스크 쓰기 완료 후 검증하여 레이스 방지)
        cache.flush()
        ...
        time.sleep(0.2)  # TTL(0.1초) 경과를 기다린 뒤 만료 확인
        expired_ttl = cache_ttl.get("ttl_prompt", model_key="test")
        T.check("TTL 만료 후 미스", expired_ttl is None)
        ...
        cache_p1.flush()  # 디스크 쓰기 완료 후 재로드 검증
        del cache_p1
