...
v47 시나리오 테스트(test_scenarios.py)에 포함되지 않은 고유 테스트:
...
# 프로젝트 루트 추가 (tests/ 하위에서 실행되므로 상위 디렉토리)
...
class TestRunner:
    """테스트 결과 수집 및 보고"""
    __test__ = False  # pytest가 테스트 클래스로 수집하지 않도록 (테스트 유틸리티)
    def __init__(self):
...
def test_agent_v4():
    ...
    # 기본 초기화 (구성된 Azure Search 인덱스가 서비스에 없으면 통합 검증은 건너뜀)
    try:
        agent = KoreanDocAgent()
    except ResourceNotFoundError as e:
        T.skip("Agent 초기화", f"Azure Search 인덱스 없음 (인프라 미준비): {e}")
        import inspect
        sig = inspect.signature(KoreanDocAgent.__init__)
        T.check("KoreanDocAgent class importable", True)
        T.check("__init__ has graph_manager param", "graph_manager" in sig.parameters)
...
def test_guardrail_scenarios():
    ...
    # v6.0 속성 (async 파이프라인 / agentic retrieval — 가드레일 경로에서는 미사용이나
    # 실제 __init__와 동기화하여 향후 코드가 접근해도 AttributeError가 나지 않도록 설정)
