...
class LLMResponseCache:
    ...
    def flush(self) -> None:
        """대기 중인 비동기 디스크 쓰기가 모두 완료될 때까지 대기합니다.

        테스트/종료 시점에 호출하면 디스크 영속성 확인이나 디렉토리 정리 시
        백그라운드 daemon 쓰기와의 레이스(특히 Windows 파일 잠금)를 방지합니다.
        """
        if self.enabled:
    ...
    def clear(self):
        """모든 캐시를 초기화합니다."""
        # 비동기 디스크 쓰기를 먼저 비운 뒤 삭제하여 daemon 쓰기와의 레이스를 방지
        self.flush()
        with self._lock:
