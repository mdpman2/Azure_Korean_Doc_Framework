...
def classify_error(exc: Exception) -> ErrorClass:
    ...
    # 컨텍스트 초과는 보통 400(BadRequest)+특정 코드이므로 메시지로 우선 판정합니다.
    if "context_length_exceeded" in exc_str or "too many tokens" in exc_str or "maximum context" in exc_str:
        return ErrorClass.CONTEXT_OVERFLOW

    # openai>=1.x 타입 예외: HTTP status_code로 정확히 분류
    # (str(exc) 부분매칭은 "1500" 같은 토큰 수에 '500'이 걸리는 오탐이 있어 우선 사용)
