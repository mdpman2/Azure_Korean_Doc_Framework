...
class KoreanUnicodeTokenizer:
    ...
    def find_text_positions(
        ...
        if not positions and fuzzy:
            ...
            if idx != -1:
                # 시작/끝 위치를 모두 정규화 좌표 기준으로 원문에 역매핑합니다.
                # (기존: end_pos에 원본 search_text 길이를 더해 공백 축약 시 구간이 어긋남)
                orig_idx = _map_normalized_to_original(source_text, idx)
                orig_end = _map_normalized_to_original(source_text, idx + len(normalized_search))
                if orig_idx >= 0:
                    positions.append(CharInterval(
                        start_pos=orig_idx,
                        end_pos=orig_end if orig_end > orig_idx else orig_idx + len(search_text),
                    ))
