...
> **2026-03 문서 정리**: 현재 코드 기준으로 Azure AI Search 필드 매핑, exact citation, 레이아웃 메타데이터, 테스트 현황을 다시 맞췄습니다.
...
- **운영 포인트**: 기존 Azure AI Search 인덱스 재사용, exact citation, diagnostics, 라이브 재색인 검증
...
├── test_framework_v4.py         # 종합 테스트 스위트 (현재 220개 항목 검증)
...
- `python test_framework_v4.py`: 220 통과 / 0 스킵 / 0 실패
...
======================================================================
  ...
  [9] Agent v4.1: 21/21
  [10] Guardrails v4.2: 10/10
  [11] Guardrail Scenarios: 14/14
  [12] ChunkLogger: 10/10
  [13] VectorStore v4.1: 5/5
  [14] CLI v4.0 Args: 19/19

총 결과: 220 통과 / 0 스킵 / 0 실패 (총 220개)
모든 코드 테스트 통과
```
...
- ✅ **회귀 테스트 보강**
  ...
  - `test_framework_v4.py` 기준 220 통과 / 0 스킵 / 0 실패
- ✅ **재검증 기록 정리**
